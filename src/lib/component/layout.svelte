<script lang="ts">
	...
	clientAddress = invoiceRef.client.address.split('\n')
...
<header>
	<div class="left">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 75 46" aria-labelledby="logoTitle">
		...
		<a target="_blank" href="https://jujiyangasli.com" aria-label="juji's site"></a>
	</div>
	<div class="right">
		...
		<br />
		<p>Tri Rahmat Gunadi</p>
		<a target="_blank" href="https://jujiyangasli.com">jujiyangasli.com</a>
...
<main>
	...
	<div class="bottom">
		<div class="address">
			<p class="address-title">This {title.toLowerCase()} is addresed to:</p>
			<p class="entities"><b>{clientName}</b></p>
			<address>
				{#each clientAddress as line}
					<p>{line}</p>
				{/each}
			</address>
			<p class="entities"><small>{clientPerson}</small></p>
			<p class="entities"><small>
				<a target="_blank" href={`mailto:${clientPersonEmail}`}>
					{clientPersonEmail}</a>
				</small></p>
		</div>
		{#if title === 'Receipt' }
		...
		{:else}
			<div class="payment-info">
				<p><b>Payment(s) can be done with Paypal:</b></p>
				<p>
					<a target="_blank" href={singlePaymentUrl}>Single Payment</a>
					If you want to do a single payment.
				</p>
				<p>
					<a target="_blank" href={subscriptionUrl}>Subscription</a>
					If you whish to subscribe. You can cancel anytime.
				</p>
			</div>
		{/if}
	</div>
